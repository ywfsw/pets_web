...
<template>
  <n-modal
    ...
    :bordered="false"
    :mask-closable="false"
    class="feeding-form-modal-wrap"
    transform-origin="center"
  >
    <div class="form-glass-card form-glass-card--feeding">
      <!-- 浮动装饰 -->
      <div class="form-decor form-decor-1">🍖</div>
      <div class="form-decor form-decor-2">✨</div>
      <div class="form-decor form-decor-3">🦴</div>
      <div class="form-decor form-decor-4">🍽️</div>
      ...
      <div class="form-header">
        <div class="form-header-icon form-header-icon--feeding">
          <span style="font-size: 28px;">🍖</span>
        </div>
        <h2 class="form-title form-title--feeding">{{ petStore.feedingRecordFormModal.isEdit ? '编辑喂养记录' : '记录喂养' }}</h2>
        <p class="form-subtitle">{{ petStore.feedingRecordFormModal.isEdit ? '更新喂养信息' : '记录宠物饮食详情 🐾' }}</p>
      </div>

      <!-- 表单内容 -->
      <div v-if="petStore.feedingRecordFormModal.data" class="form-body">
        <n-form ref="formRef" @submit.prevent="handleSubmit" label-placement="top" :rules="rules">
          <n-form-item label="喂食时间" path="feedTime" required :label-style="{ fontWeight: '600', color: 'var(--pet-text)' }">
            <n-date-picker
          ...
          <n-form-item label="食物类型" path="foodType" required :label-style="{ fontWeight: '600', color: 'var(--pet-text)' }">
            <n-input
          ...
          <div class="form-row">
            <n-form-item label="食量 (克)" class="form-row-item" :label-style="{ fontWeight: '600', color: 'var(--pet-text)' }">
              <n-input-number
      ...
      <div class="form-footer">
        <n-button @click="handleClose" size="large" class="cancel-btn" :disabled="petStore.feedingRecordFormModal.loading">
          取消
        </n-button>
        <n-button type="primary" @click="handleSubmit" :loading="petStore.feedingRecordFormModal.loading" size="large" class="submit-btn submit-btn--feeding">
...
<style scoped>
/* 模态框外层 */
.feeding-form-modal-wrap :deep(.n-modal-mask) {
  background: rgba(0, 0, 0, 0.5) !important;
  backdrop-filter: blur(6px);
}

.feeding-form-modal-wrap :deep(.n-modal-body-wrapper) {
  display: flex;
  align-items: center;
...
.feeding-form-modal-wrap :deep(.n-modal) {
...
.form-glass-card::-webkit-scrollbar-thumb { background: rgba(245, 158, 11, 0.2); border-radius: 4px; }
...
.form-body :deep(.n-input--focus) {
  box-shadow: 0 0 0 3px rgba(245, 158, 11, 0.15), 0 2px 12px rgba(245, 158, 11, 0.12);
}
